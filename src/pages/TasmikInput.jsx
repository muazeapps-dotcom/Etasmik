...
import { Save, User, BookOpen, CheckCircle, Search, GraduationCap, AlertCircle, Loader2 } from 'lucide-react';
...
function TasmikInput() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedClass || !studentName || !level || !pageNumber) {
      alert("⚠️ Sila isi semua maklumat murid (Nama, Tahap & Muka Surat).");
      return;
    }

    setLoading(true);
    try {
      // Data yang dihantar mesti sepadan dengan SQL tasmik_records
      const dataToInsert = {
        student_name: `${studentName} (${selectedClass})`,
        reading_type: readingType,
        level: level.toString(),
        page_number: parseInt(pageNumber), // Mesti tukar ke nombor (integer)
        remarks: remarks || '-',
      ...
      alert(`✅ Berjaya! Rekod tasmik ${studentName} telah disimpan.`);
      
      // Reset input selepas berjaya
      setStudentName('');
      setSearchMurid('');
      setLevel('');
      setPageNumber('');
    ...
    <div className="p-4 max-w-2xl mx-auto mb-24 min-h-screen">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 font-bold text-xs uppercase tracking-widest">Rekod Bacaan Murid</p>
        </div>
      </div>
          ...
          {CLASSES.map((c) => (
            <button
              ...
              className={`py-4 rounded-2xl font-black text-xs transition-all transform active:scale-95 border-2 ${
                ...
                : 'bg-white border-gray-100 text-gray-400 hover:border-green-200 shadow-sm'
              }`}
            >
              {c}
            </button>
      ...
      {selectedClass && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-gray-50">
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 block italic text-center sm:text-left">Langkah 2: Pilih Nama Murid ({selectedClass})</label>
            
            <div className="relative mb-4">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" size={18} />
              <input 
                type="text"
                placeholder="Taip nama untuk cari..."
            ...
            <div className="max-h-64 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
              {filteredStudents.length > 0 ? filteredStudents.map((name) => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl font-bold text-[11px] flex items-center justify-between transition-all ${
                    studentName === name 
                ...
                <div className="text-center py-6 text-gray-400 italic flex flex-col items-center">
                   <AlertCircle className="mb-2 opacity-30" size={32} />
                   <p className="text-xs">Nama tidak ditemui...</p>
                </div>
              )}
            </div>
          </div>
          ...
          {studentName && (
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-gray-50 space-y-5">
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="text-[10px] font-black text-gray-400 uppercase ml-2 mb-1 block tracking-tighter">Jenis Bacaan</span>
                    <select 
                  ...
                  <div>
                    <span className="text-[10px] font-black text-gray-400 uppercase ml-2 mb-1 block tracking-tighter">
                      {readingType === 'Iqra' ? 'Tahap (1-6)' : 'Juz (1-30)'}
                    </span>
                    <input 
                      type="text"
                      placeholder={readingType === 'Iqra' ? "Cth: 2" : "Cth: 15"}
               ...
               <div>
                  <span className="text-[10px] font-black text-gray-400 uppercase ml-2 mb-1 block tracking-tighter">Nombor Halaman (Muka Surat)</span>
                  <input 
                    type="number"
               ...
               <div>
                  <span className="text-[10px] font-black text-gray-400 uppercase ml-2 mb-1 block tracking-tighter">Catatan / Maklum Balas</span>
                  <input 
                    type="text"
                    placeholder="Contoh: Bacaan lancar, perlu ulang."
